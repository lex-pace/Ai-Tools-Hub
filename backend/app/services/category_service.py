...
async def _count_tools_for_categories(
    ...
    stmt = (
        ...
        .where(Tool.category_id.in_(category_ids), Tool.status == "active")
...
async def get_category_tree(
    ...
    if parent_id is None:
        ...
        count_map = await _count_tools_for_categories(db, cat_ids)

        items = []
        for c in categories:
            c_dict = _category_to_dict(c)
            c_dict["tool_count"] = count_map.get(c.id, 0)
...
async def get_category_by_id(
    ...
    count_map = await _count_tools_for_categories(db, [category_id])
    cat_dict = _category_to_dict(category)
    cat_dict["tool_count"] = count_map.get(category_id, 0)
    if category.children:
        child_ids = [ch.id for ch in category.children]
        child_counts = await _count_tools_for_categories(db, child_ids)
        cat_dict["children"] = []
        for ch in category.children:
            ch_dict = _category_to_dict(ch)
            ch_dict["tool_count"] = child_counts.get(ch.id, 0)
            cat_dict["children"].append(CategoryOut(**ch_dict))
